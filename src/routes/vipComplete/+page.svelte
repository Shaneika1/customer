...
{#if loading == false}
    <section class="guest-order-section bg-center bg-no-repeat">
        <div class="px-4 mx-auto text-center pt-10 lg:py-16">
            ...
            >
               {#if status == 'success' } Your Subscription was Successful {/if}
               {#if status == 'cancelled' } Your Subscription was Unsuccessful {/if}
            </h1>
            <p
                class="mb-8 text-lg font-normal text-gray-700 lg:text-xl sm:px-16 lg:px-48 normal-text"
            >
                For more information, check out your profile.
            </p>
        </div>
        <div class="mx-1 p-10" style=""></div>
    </section>
{/if}
